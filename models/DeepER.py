...
def init_embeddings_index(embeddings_file):
    ...
    logging.info('* Costruzione indice degli embeddings.....',
                 end='', flush=True)
    embeddings_index = {}
    ...
    logging.info(f'Fatto. {len(embeddings_index)} embeddings totali.')
...
def init_embeddings_model(embeddings_index):
    ...
    logging.info(
        '* Creazione del modello per il calcolo degli embeddings....', flush=True)

    logging.info('* Inizializzo il tokenizzatore.....', end='', flush=True)
    tokenizer = Tokenizer(filters='')
    tokenizer.fit_on_texts(embeddings_index.keys())
    words_index = tokenizer.word_index
    logging.info(f'Fatto: {len(words_index)} parole totali.')

    logging.info('* Preparazione della matrice di embedding.....',
                 end='', flush=True)
    ...
    logging.info(
        f'Fatto. Dimensioni matrice embeddings: {embedding_matrix.shape}')
    ...
    embeddings_model = Model(inputs=[input_a, input_b], outputs=[
...
def init_DeepER_model(embedding_dim):
    ...
    # Hadamard
    similarity = Lambda(
        lambda ts: ts[0] * ts[1], name='Similarity')([lstm_a, lstm_b])
...
def data2Inputs(data, tokenizer, categorical=True):
    ...
    logging.info('* Preparazione input......', end='', flush=True)
...
def train_model_ER(data, model, embeddings_model, tokenizer, pretraining=False, metric='val_accuracy', end=''):
    ...
    if pretraining:
        model_name = 'VinSim'
        table1, table2, labels = data2Inputs(
            data, tokenizer, categorical=False)
    else:
        model_name = 'DeepER'
    ...
    mc = ModelCheckpoint(f'{model_name}_best_model{end}.h5',
                         monitor=metric, verbose=1, save_best_only=True)
    # Addestramento modello
    param_batch_size = round(len(data) * 0.015) + 1
    logging.info('Batch size:', param_batch_size)
    model.fit([x1, x2], labels, batch_size=param_batch_size,
...
def model_statistics(data, model, embeddings_model, tokenizer):
    ...
    logging.info('* Avvio test metriche....', flush=True)
    ...
    logging.info(f'-- Non Match: {no_match_count}')
    logging.info(f'-- Match: {match_count}')
    ...
    logging.info('* Evaluating: ', end='', flush=True)
    ...
    try:
        ...
        logging.info(
            f'Precision: {precision}, Recall: {recall}, f1-score: {fmeasure}')
        logging.info(
            f'Total retrieved: {retrieved_size}, retrieved/total matches: {true_match}/{match_count}')

    except:
        logging.info(
            f'Error. Retrieved = {retrieved_size}, Matches = {match_count} ')
        precision = -1
        recall = -1
...
def model_statistics_prf(data, model, embeddings_model, tokenizer):
    ...
    logging.info('-- Corpus size: {a}'.format(a=corpus_size))
    logging.info('-- Non Match: {a}'.format(a=no_match_count))
    logging.info('-- Match: {a}'.format(a=match_count))
    ...
    try:
        ...
        logging.info(
            'Precision: {a}, Recall: {b}, f1-score: {c}'.format(a=precision, b=recall, c=fmeasure))
        logging.info('Total retrieved: {a}, retrieved/total matches: {b}/{c}'.format(
            a=retrieved_size, b=true_match, c=match_count))

    except:

        logging.info('Error. Retrieved = {a}, Matches = {b} '.format(
            a=retrieved_size, b=match_count))
        precision = -1
        recall = -1
...
def model_statisticsNOMatch_prf(data, model, embeddings_model, tokenizer):
    ...
    try:
        ...
        logging.info(
            'Precision: {a}, Recall: {b}, f1-score: {c}'.format(a=precision, b=recall, c=fmeasure))
        logging.info('Total retrieved: {a}, retrieved/total no_matches: {b}/{c}'.format(
            a=retrieved_size, b=true_NOmatch, c=no_match_count))
